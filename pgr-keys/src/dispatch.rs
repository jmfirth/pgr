...
impl<R: Read, W: Write> Pager<R, W> {
    ...
    fn repaint(&mut self) -> Result<()> {
        ...
        // A file is "short" only when the entire file fits within one screen,
        // not merely when the viewport extends past EOF due to search positioning.
        let visible_content = total.saturating_sub(start);
        let short_file = total <= content_rows;
