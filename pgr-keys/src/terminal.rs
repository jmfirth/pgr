...
/// Escape sequence to enable application keypad mode (smkx).
///
/// Sent to the terminal to switch the numeric keypad into application mode,
/// which changes the escape sequences generated by arrow and function keys.
pub const KEYPAD_ENABLE: &[u8] = b"\x1b[?1h";

/// Escape sequence to disable application keypad mode (rmkx).
///
/// Sent on pager exit to restore normal keypad behavior.
pub const KEYPAD_DISABLE: &[u8] = b"\x1b[?1l";
...
mod tests {
    ...
    #[test]
    fn test_keypad_enable_sequence_is_correct() {
        assert_eq!(KEYPAD_ENABLE, b"\x1b[?1h");
    }

    #[test]
    fn test_keypad_disable_sequence_is_correct() {
        assert_eq!(KEYPAD_DISABLE, b"\x1b[?1l");
    }
